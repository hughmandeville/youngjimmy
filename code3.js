var yt_ids = ["xK6mYENLsfo", "DuwOr4_74cg", "qF_HVIrmcsY", "nfKMVYzotT8"];
var yt_id = 0;
...
$(function () {
  ...
  $("#button_video_previous").on("click", function () {
    yt_id--;
    if (yt_id < 0) {
      yt_id = yt_ids.length - 1;
    }
    setVideo();
  });
  $("#button_video_next").on("click", function () {
    yt_id++;
    if (yt_id >= yt_ids.length) {
      yt_id = 0;
    }
    setVideo();
  });
...
function setVideo() {
  $("#video").attr("src", "https://www.youtube.com/embed/" + yt_ids[yt_id]);
}
